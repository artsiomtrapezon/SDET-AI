...
for (const record of dataset) {
  const name = `Echo validates order ${record.orderId} (isValid=${record.isValid})`;
  ...
  test(name, async ({ request }) => {
    ...
    // --- Structural checks ---
    const requiredKeys = ["orderId","customerName","email","phoneNumber","status","totalAmount","items","isValid"];
    ...
    if (record.isValid) expect(typeof record.totalAmount).toBe("number");

    // --- Pattern & range checks (only for valid records) ---
    if (record.isValid) {
      ...
      // For intentionally invalid rows, assert the presence of a violation hint if included.
      // Example: expect(record.why).toBeDefined();
    }
  });
}